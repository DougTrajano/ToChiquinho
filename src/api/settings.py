...
class Settings(BaseSettings):
    ...
    API_ROUTE_TOXICITY_ENDPOINT: str = Field(
        None, description="API endpoint to toxicity classification. (e.g. http://localhost:8002/predict)")
    API_ROUTE_TOXICITY_TARGET_ENDPOINT: str = Field(
        None, description="API endpoint to toxicity target classification. (e.g. http://localhost:8004/predict)")
    API_ROUTE_TOXICITY_TARGET_TYPE_ENDPOINT: str = Field(
        None, description="API endpoint to toxicity target type identification. (e.g. http://localhost:8005/predict)")
    API_ROUTE_TOXICITY_TYPE_ENDPOINT: str = Field(
        None, description="API endpoint to toxicity type detection. (e.g. http://localhost:8003/predict)")
    API_ROUTE_TOXIC_SPANS_ENDPOINT: str = Field(
        None, description="API endpoint to toxic spans detection (e.g. http://localhost:8001/predict)")
    ...
    def validate_api_task(cls, v):
        ...
        if v == "route":
            if not cls.API_ROUTE_TOXIC_SPANS_ENDPOINT:
                raise ValueError(
                    "API_ROUTE_TOXIC_SPANS_ENDPOINT must be set.")
            if not cls.API_ROUTE_TOXICITY_ENDPOINT:
                raise ValueError(
                    "API_ROUTE_TOXICITY_ENDPOINT must be set.")
